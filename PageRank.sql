...
score = (select * from :score) union (select n.dest as dest, sum(n.score/c.count) as score from :new_graph as n, :dest_count as c where n.src = c.src group by n.dest);
new_graph = (select * from :new_graph) union (select m.src, m.dest, s.score from :new_graph as m, :score as s where m.src = s.dest);
